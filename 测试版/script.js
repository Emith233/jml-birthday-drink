...
const bgm=$('#bgm');
const preloadAssets=['../素材/角色/比格酒保-忙碌转场.webp','../素材/角色/比格酒保-摇酒-统一背景.webp','../素材/场景/01-小猪客人.webp','../素材/场景/02-德文猫客人.webp','../素材/场景/03-小熊客人.webp','../素材/场景/04-比格酒保庆祝.webp','../素材/场景/05-最终庆祝.webp','../素材/调酒材料/调饮制作台.webp',...materials.map(m=>`../素材/调酒材料/独立版/${m[2]}`)];
function preload(){const total=preloadAssets.length+1;let loaded=0;const update=()=>{loaded++;const percent=Math.round(loaded/total*100);$('#bootProgress').style.width=`${percent}%`;$('#bootPercent').textContent=`${percent}%（${loaded}/${total}）`;$('#bootStatus').textContent=percent<100?'正在准备第一杯心意……':'准备好了，店长。'};const imageJobs=preloadAssets.map(src=>new Promise(resolve=>{const img=new Image();img.onload=()=>{update();resolve()};img.onerror=()=>{update();resolve()};img.src=src}));const fontJob=document.fonts?.load("16px '酒馆涂鸦体'").then(update,update)||Promise.resolve(update());Promise.all([...imageJobs,fontJob]).then(()=>{setTimeout(()=>{$('#bootLoader').classList.add('ready');const btn=$('#beginBtn');btn.disabled=false;btn.classList.remove('disabled');btn.innerHTML='开门 <span>→</span>'},220)})}
preload();
